...
func main() {
	...
	groundchannel = make(chan wavegen.Sample, parameters.CHANSIZE)
	groundX = make([]float32, 0)
	groundY = make([]float32, 0)

	subchannel = make(chan wavegen.Sample, parameters.CHANSIZE)
	subX = make([]float32, 0)
	subY = make([]float32, 0)

	predchannel = make(chan wavegen.Sample, parameters.CHANSIZE)
	predX = make([]float32, 0)
	predY = make([]float32, 0)

	biaschannel = make(chan wavegen.Sample, parameters.CHANSIZE)
