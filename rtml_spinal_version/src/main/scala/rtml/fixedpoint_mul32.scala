...
class fixedpoint_mul32(core_clock: ClockDomain = ClockDomain.current) extends Component {
  val io = new Bundle {
    //val aclk                = in  Bool
    //val aclken              = in  Bool
    //val aresetn             = in  Bool
...
object fixedpoint_mul32_TopVerilog {
  def main(args: Array[String]) {
    SpinalVerilog(new fixedpoint_mul32())
  }
}
