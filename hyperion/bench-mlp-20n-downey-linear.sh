#!/bin/bash
#SBATCH --job-name=mlp-20n-downey-linear
#SBATCH -o %j_mlp20nlinear_downey.out
#SBATCH -e %j_mlp20nlinear_downey.err
#SBATCH --ntasks=252
        ...
        for w in {20..100..10}; do
            ...
            for h in {10..100..10}; do
                ...
                for x in {1..10}; do
                    ...
                    if (( `python3 dbtool.py query results.db "SELECT * FROM AlgorithmResult WHERE dataset=\"$(basename $dataset).json\" AND layers=\"20\" AND epochs=$e AND training_window_length=$w AND sample_window_length=$h AND activation=\"linear\"" | wc -l` >= 11 )); then
                        break
                    fi
                    srun -n1 -c1 --exclusive ./job.sh -u 20 -s ${h} -t ${w} -e ${e} -f "$(basename $dataset).json" | python3 dbtool.py insert results.db &
                done
            done
        done
    done
