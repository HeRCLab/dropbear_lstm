#!/bin/bash
#SBATCH --job-name=mlp-10n-downey-relu
#SBATCH -o %j_mlp10nrelu_downey.out
#SBATCH -e %j_mlp10nrelu_downey.err
#SBATCH --ntasks=252
        ...
        for w in {20..100..10}; do
            ...
            for h in {10..100..10}; do
                ...
                for x in {1..10}; do
                    ...
                    if (( `python3 dbtool.py query results.db "SELECT * FROM AlgorithmResult WHERE dataset=\"$(basename $dataset).json\" AND layers=\"10\" AND epochs=$e AND training_window_length=$w AND sample_window_length=$h AND activation=\"relu\"" | wc -l` >= 11 )); then
                        break
                    fi
